module PolyAES where

import Data.Bits
import Data.Array
import Poly8

newtype AesState = AesState (Array (Int, Int) Poly8)
    deriving (Eq, Show)

mkAesState :: [Poly8] -> AesState
mkAesState = AesState .  array ((0,0),(3,3)) . zip [(r, c) | c <- [0..3], r <- [0..3]]

fromAesState :: AesState -> [Poly8]
...
keyExpansion :: [Poly8] -> Array Int AesState
keyExpansion ks = listArray (0, 10) . map (mkAesState . concat) $ groupN 4 ws
    where
        rcs = 1 : map (\rc -> if rc < 0x80 then 2*rc else ((2 * rc) `xor` 0x1b) .&. 0xff) rcs :: [Integer]
        rcon = map (\rc -> [fromInteger rc, 0, 0, 0]) rcs
...
subByte :: Poly8 -> Poly8
subByte b = sum $ zipWith (\v r -> eqMask b (fromInteger v) .&. r) [0..] aesSBox

subBytes :: AesState -> AesState
subBytes (AesState bytes) = AesState $ fmap subByte bytes

shiftRows :: AesState -> AesState
shiftRows (AesState bytes) = AesState $ ixmap (bounds bytes)
                             (\(r, c) -> (r, mod (c + r) 4)) bytes

mixColumn :: [Poly8] -> [Poly8]
mixColumn a = map (foldr1 xor) [
               ...
               [b !! 3, a !! 2, a !! 1, b !! 0, a !! 0]]
    where
        b = map (\x -> (x `shiftL` 1) `xor` (0x1b .&. (x `shiftR` 7))) a
